// Original Pings renderer by Azzurite; modified 2026-09-20 for lifecycle cleanup and bounded animation values.
/**
...
export default class Ping extends PIXI.Container {
	...
	constructor(foundryCanvas, foundryConfig, pos, id, text, color, options) {
        ...
        for (const [key, fallback, min, max] of [
          ['scale', 1, 0.25, 5], ['duration', 6, 0.5, 30],
          ['rotateSpeed', 6, 0.5, 20], ['sizeChangeAmount', 0.125, 0, 0.5], ['sizeChangeSpeed', 3, 0.5, 10]
        ]) {
          const n = Number(this.options[key]);
          this.options[key] = Number.isFinite(n) ? Math.max(min, Math.min(max, n)) : fallback;
        }
        if (foundryCanvas.photosensitiveMode) {
          this.options.rotate = false;
          this.options.sizeChange = false;
        }
        this.eventMode = 'none';

		const gridSize = foundryCanvas.scene.grid.size || 100;
		...
		this._ticker = foundryCanvas.app.ticker;
		this._ticker.add(this._animateFunc, this);
	}

	destroy(options) {
		if (this.destroyed) return;
		this._ticker.remove(this._animateFunc, this);
